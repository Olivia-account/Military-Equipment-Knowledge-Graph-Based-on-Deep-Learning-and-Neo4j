    ...
    for spo in spo_list:
        for spo_object in spo['object'].keys():
            ...
            if spo['predicate'] in label_map.keys():
                ...
                label_object = label_subject + 24 # 119
                subject_tokens = tokenizer._tokenize(spo['subject'])
            ...
            if subject_tokens_len > object_tokens_len:
                for index in range(seq_len - subject_tokens_len + 1):
                    if tokens[index:index + subject_tokens_len] == subject_tokens:
                        #print("第{}到{}的序列有标记".format(index,index + subject_tokens_len-1))
                        #print('1 and 2',tokens[index:index + subject_tokens_len] ,subject_tokens)
                        labels[index][label_subject] = 1 # =1
                        for i in range(subject_tokens_len - 1):
                            labels[index + i + 1][1] = 1 #  疑问
                            #labels[index + i + 1][label_subject] = 5# 补充
                    ...
                    if tokens[index:index + object_tokens_len] == object_tokens:
                        if forbidden_index is None:
                            ...
                            for i in range(object_tokens_len - 1):
                                labels[index + i + 1][1] = 1 # label_object
                                #labels[index + i + 1][label_object] = 5
                            break
                        # check if labeled already
                            ...
                            for i in range(object_tokens_len - 1):
                                ...
                                #labels[index + i + 1][label_object] = 5  # label_object
                            break
...
def convert_example_to_feature(
        ...
        label_map, seq_label_map,
        max_length: Optional[int]=512,
        pad_to_max_length: Optional[bool]=None):
    spo_list = example['spo_list'] if "spo_list" in example.keys() else None
    text_raw = example['text']
    seq_label = [0 for i in range(100)]
    if "label" in example.keys():
        seq_label_id=seq_label_map[example['label']]
        seq_label[seq_label_id]=1
    sub_text = []
    buff = ""
    ...
    return InputFeature(
        ...
        labels=np.array(labels),  #[128,240]
        seq_labels=np.array([seq_label]))
...
class DuIEDataset(paddle.io.Dataset):
    ...
    def __init__(
            ...
            seq_labels: List[Union[List[int], np.ndarray, List[str], List[Dict]]]
    ):
    ...
    def __getitem__(self, item):
        return {
            ...
            "seq_labels": np.array(
                self.seq_labels[item], dtype=np.float32),
        }
    ...
    def from_file(cls,
        ...
        with open(label_map_path, 'r', encoding='utf8') as fp:  # 生成label_map
            label_map = json.load(fp)
        ##############################句子分类的map################################
        seq_label_map_path = os.path.join(
            os.path.dirname(file_path), "classes2id.json")
        assert os.path.exists(label_map_path) and os.path.isfile(
            seq_label_map_path
        ), f"{seq_label_map_path} dose not exists or is not a file."
        with open(seq_label_map_path, 'r', encoding='utf8') as fp:  # 生成label_map
            seq_label_map = json.load(fp)
        chineseandpunctuationextractor = ChineseAndPunctuationExtractor()

        input_ids, seq_lens, tok_to_orig_start_index, tok_to_orig_end_index, labels,seq_labels = (
            [] for _ in range(6)) #5
        dataset_scale = sum(1 for line in open(file_path, 'r',encoding='utf-8'))
        ...
        with open(file_path, "r", encoding="utf-8") as fp:
            ...
            for line in tqdm(lines):
                ...
                input_feature = convert_example_to_feature(
                    ...
                    label_map, seq_label_map, max_length, pad_to_max_length)
                ...
                seq_labels.append(input_feature.seq_labels)
        return cls(input_ids, seq_lens, tok_to_orig_start_index,
                   tok_to_orig_end_index, labels, seq_labels)
...
class DataCollator:
    ...
    def __call__(self, examples: List[Dict[str, Union[list, np.ndarray]]]):
        ...
        labels_seq=np.stack([x['seq_labels'] for x in examples])

        return (batched_input_ids, seq_lens, tok_to_orig_start_index,
                tok_to_orig_end_index, labels,labels_seq)


if __name__ == "__main__":
    #tokenizer = ErnieTokenizer.from_pretrained("ernie-1.0")
    tokenizer = AutoTokenizer.from_pretrained('ernie-3.0-medium-zh')
    d = DuIEDataset.from_file("./data/train_1000.json", tokenizer)
