...
def find_entity(text_raw, id_, predictions, tok_to_orig_start_index,
    ...
    for i in range(len(predictions)):
        #print("len_predictions",len(predictions))
        if [id_] in predictions[i]:
            #print("predcition[i]:",predictions[i])
            j = 0
...
def decoding(example_batch,
    ...
    """
    formatted_outputs = []
    for (i, (example, logits, logits_seq, seq_len, tok_to_orig_start_index, tok_to_orig_end_index)) in \
            enumerate(zip(example_batch, logits_batch,logits_seq_batch, seq_len_batch, tok_to_orig_start_index_batch, tok_to_orig_end_index_batch)):

        seq_cls=id2cls[int(np.argmax(logits_seq))]
        ...
        subject_id_list = []
        #print("没展开的预测：",predictions)
        #print("展开的预测：",flatten_predictions)
        for cls_label in list(set(flatten_predictions)):
            if 1 < cls_label <= 24 and (cls_label + 24) in flatten_predictions:  # 56 55 119
        ...
        for id_ in subject_id_list:
            ...
            if id_ not in complex_relation_label:
                subjects = find_entity(text_raw, id_, predictions,
                                       ...
                                       tok_to_orig_end_index)
                objects = find_entity(text_raw, id_ + 24, predictions, #55
                                      tok_to_orig_start_index,
                                      tok_to_orig_end_index)
                #print("sub n obj:",subjects,objects)
                ...
                subjects = find_entity(text_raw, id_, predictions,
                                       tok_to_orig_start_index,
                                       tok_to_orig_end_index)
                objects = find_entity(text_raw, id_ + 24, predictions,
                                      tok_to_orig_start_index,
                                      tok_to_orig_end_index)
                for subject_ in subjects:
                    for object_ in objects:
                        ...
                        if id_ in [#8, 10, 32, 46
                            ...
                            object_dict[id2spo['object_type'][id_affi].split(
                                '_')[1]] = find_entity(text_raw, id_affi + 24,
                                                       predictions,
                                                       tok_to_orig_start_index,
                            ...
                            for id_affi in [27, 28, 29]:
                                if id_affi in subject_id_list:
                                    ...
                                    find_entity(text_raw, id_affi + 24, predictions, tok_to_orig_start_index, tok_to_orig_end_index)[0]
        ...
        formatted_instance['label'] = seq_cls
        if 'id' in example:
            formatted_instance['id']=example['id']
